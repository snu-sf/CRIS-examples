...
Module ClientI. Section ClientI.
  ...
  Definition thread : list val → itree crisE unit :=
    λ arg,
      𝒴;;; '_ : val <- ccallU (cftyp _ _) IncrHdr.incr arg;;
      𝒴;;; '_ : val <- ccallU (cftyp _ _) IncrHdr.incr arg;;
      𝒴;;; Ret tt.

  Definition main : Any.t → itree crisE Any.t :=
    λ _,
      𝒴;;; 'ptr_raw : val <- ccallU (cftyp _ _) MemHdr.alloc [Vint 1%Z];;
      𝒴;;; bofs <- (pargs [Tptr] [ptr_raw])?;;
      𝒴;;; '_ : val <- ccallU (cftyp _ _) MemHdr.store [Vptr bofs; Vint 0%Z];;
  ...
  Definition fnsems : fnsemmap :=
    {[fid ClientHdr.thread # (msk_real (msk_scp scopes msk_true), (None, cfunU (cftyp _ _) (sfunU (_,_) thread)));
