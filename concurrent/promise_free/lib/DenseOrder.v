...
Module DOAux.
  ...
  Proof.
    ...
    - apply le_lteq in l. apply le_lteq in l0.
      des; auto.
      destruct lt_strorder as [IRREF TRANS].
      exfalso.
      apply (IRREF lhs); eapply TRANS with (y := rhs); assumption.
    - destruct lt_strorder as [IRREF TRANS].
      exfalso.
      apply (IRREF lhs); eapply TRANS with (y := rhs); assumption.
  Qed.
    ...
    repeat
      (try match goal with
           ...
           | [H1: lt ?a ?b, H2: lt ?b ?a |- _] =>
              destruct lt_strorder as [IRREF TRANS];
              exfalso; 
              apply (IRREF a); eapply TRANS with (y := b); assumption
           end; unfold eq in *; try subst; des; ss).
    - destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF a); eapply TRANS with (y := b).
      + assumption.
      + eapply TRANS with (y := c); [exact l | exact l0].
    - destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF b); eapply TRANS with (y := a).
      + assumption.
      + eapply TRANS with (y := c); [exact l | exact l1].
  Qed.
  ...
  Proof.
    ...
    - apply le_lteq in l. apply le_lteq in l0.
      des; auto.
      destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF lhs); eapply TRANS with (y := rhs); assumption.
    - destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF lhs); eapply TRANS with (y := rhs); assumption.
  Qed.
  ...
  Proof.
    ...
    - destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF a); eapply TRANS with (y := b).
      + assumption.
      + eapply TRANS with (y := c); [exact l1 | exact l].
    - destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF b); eapply TRANS with (y := a).
      + assumption.
      + eapply TRANS with (y := c); [exact l0 | exact l1].
  Qed.
...
Definition _DOAux_eq_transitive : Transitive DOAux.eq := Equivalence_Transitive.
#[export]
...
Next Obligation.
  ...
  des; subst; auto.
  left. destruct DOAux.lt_strorder as [IRREF TRANS].
...
Module DenseOrder.
  ...
  Proof.
    ...
    - destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF a); eapply TRANS with (y := b).
      + assumption.
      + eapply TRANS with (y := c); [exact l1 | exact l0].
    - destruct lt_strorder as [IRREF TRANS].
      exfalso. apply (IRREF b); eapply TRANS with (y := a).
      + assumption.
      + eapply TRANS with (y := c); [exact l | exact l1].
...
Definition _DenseOrder_eq_transitive : Transitive DenseOrder.eq := Equivalence_Transitive.
#[export]
...
Next Obligation.
  ...
  des; subst; auto.
  left. destruct DenseOrder.lt_strorder as [IRREF TRANS].
  apply (TRANS x y z); assumption.
Qed.
...
Module DenseOrderFacts.
  ...
  Lemma le_not_lt x y:
    DenseOrder.le x y <-> ~ (DenseOrder.lt y x).
  Proof.
    split; ii.
    - assert(DenseOrder.le y x).
      { left. eauto. }
      exploit antisym; eauto. i. subst.
      eapply DenseOrder.lt_strorder_obligation_1; eauto.
    - destruct (DenseOrder.le_lt_dec x y); eauto. exfalso. eauto.
  Qed.
...
Module DOMap.
  ...
  Proof.
    ...
    { apply Raw.gleaf. }
    destruct (raw_max_key m2) eqn:K2; [congruence |].
    destruct o eqn:O; [congruence |].
    destruct (raw_max_key m1) eqn:K1; [congruence |].
    destruct k'; eauto.
  Qed.
  ...
  Proof.
    revert k KEY. induction m; s; [congruence |].
    ...
    destruct o.
    { i. inv KEY. s. splits; [congruence |].
      i. destruct k'; ss.
      - erewrite raw_max_key_spec1 in FIND'; eauto. congruence.
      - left. econs.
      - refl.
    }
  ...
  Proof.
    ...
    { splits; i.
      + rewrite PositiveMap.gleaf in H. congruence.
      + rewrite PositiveMap.gleaf in FIND'. congruence.
    }
  ...
  Lemma find_alist_elements (t : DenseOrder.t) `(m : DOMap.t A) :
        DOMap.find t m = alist_find t (DOMap.elements m).
  Proof.
    destruct (alist_find t _) eqn : FIND.
    { apply alist_find_some, DOMap.elements_complete in FIND; done. }
    { destruct (DOMap.find t m) eqn : FIND'; ss.
      eapply DOMap.elements_correct in FIND'.
      eapply alist_find_none in FIND; done.
    }
  Qed.

  (* ADDITIONAL *)
  Definition cut `(m : DOMap.t A) (t : DenseOrder.t) : DOMap.t A :=
    foldr
      (λ '(k, v) m, if (DenseOrder.le_lt_dec t k) then DOMap.add k v m else m)
      (DOMap.empty A)
      (DOMap.elements m).

  Lemma cut_find `(m : DOMap.t A) (t : DenseOrder.t) i v :
    DOMap.MapsTo i v (cut m t) → DOMap.MapsTo i v m.
  Proof.
    rewrite /DOMap.MapsTo /cut.
    intros FIND. apply DOMap.elements_complete. revert FIND.
    remember (DOMap.elements _) as l; clear Heql. induction l.
    { s. rewrite DOMap.gempty. ss. }
    { destruct a as [k val]. s. des_ifs.
      { destruct (decide (i = k)).
        { subst i; rewrite DOMap.gss; intros H; inv H; left; done. }
        { rewrite DOMap.gso; ss; intros FIND; hexploit IHl; eauto. }
      }
      intros FIND; hexploit IHl; eauto.
    }
  Qed.

  Lemma cut_spec `(m : DOMap.t A) (t t_cut : DenseOrder.t) :
    DOMap.find t (cut m t_cut) = if DenseOrder.le_lt_dec t_cut t then DOMap.find t m else None.
  Proof.
    rewrite /cut.
    rewrite (find_alist_elements t m).
    remember (DOMap.elements m) as l; clear Heql m. induction l.
    { ss. rewrite DOMap.gempty; des_ifs; ss. }
    { destruct a as [t' v].
      ...
      { s. rewrite eq_rel_dec_correct; des_ifs.
        { rewrite DOMap.gso //. }
        { rewrite DOMap.gso //. }
      }
    }
  ...
  Lemma cut_add `(m : DOMap.t A) t v t_cut :
    cut (DOMap.add t v m) t_cut =
    if DenseOrder.le_lt_dec t_cut t then DOMap.add t v (cut m t_cut) else cut m t_cut.
  Proof.
    apply DOMap.eq_leibniz; intros t'; des_ifs.
    { rewrite cut_spec; des_ifs.
      { destruct (decide (t' = t)); clarify.
        { rewrite ?DOMap.gss //. }
        { rewrite ?DOMap.gso // cut_spec; des_ifs.
          by eapply DenseOrderFacts.le_not_lt in l0.
        }
      }
      rewrite DOMap.gso; first rewrite cut_spec; des_ifs; ss.
      { exfalso; by eapply DenseOrderFacts.le_not_lt in l1. }
      ii; clarify; by eapply DenseOrderFacts.le_not_lt in l0.
    }
    rewrite ?cut_spec; des_ifs.
    rewrite DOMap.gso //.
    eapply DenseOrderFacts.le_not_lt in l0; ii; clarify.
  Qed.
End DOMap.
